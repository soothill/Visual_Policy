...
function validatePrincipalARN(principal) {
    ...
    if (/^[a-f0-9]{64}$/.test(principal)) {
        return { isValid: true, errors: [], warnings: ['Using canonical user ID - ensure this is an Impossible Cloud canonical ID'] };
    }
    ...
    if (principal.includes('.amazonaws.com') || principal.includes('.amazon.com')) {
        if (!principal.match(/^[a-z0-9-]+\.(amazonaws\.com|amazon\.com)$/)) {
            ...
            warnings.push('⚠ AWS service principals (*.amazonaws.com) may not be supported by Impossible Cloud. Use IAM user/role ARNs instead.');
            return { isValid: true, errors: [], warnings: warnings };
        }
    }
    ...
    if (principal.startsWith('arn:')) {
        ...
        if (partition === 'ipcld') {
            ...
            if (service !== 'iam') {
                errors.push('Impossible Cloud ARNs currently only support IAM service (arn:ipcld:iam::...)');
            }

            if (!canonicalId || canonicalId.length === 0) {
                errors.push('Impossible Cloud ARN requires a canonical ID (arn:ipcld:iam::YourCanonicalID:...)');
            }

            if (!resource || resource.length === 0) {
                errors.push('Impossible Cloud ARN requires a resource (e.g., user/username, policy/policyname)');
            } else if (!resource.includes('/')) {
                errors.push('Impossible Cloud resource must include type/name (e.g., user/username, policy/policyname)');
            }
        ...
        if (!arnRegex.test(principal)) {
            errors.push('Invalid ARN format. Expected: arn:aws:service:region:account-id:resource or arn:ipcld:iam::CanonicalID:resource');
        } else {
            const service = parts[2];
            ...
            if (!['aws', 'aws-cn', 'aws-us-gov'].includes(partition)) {
                errors.push(`Invalid partition "${partition}". Must be: aws, aws-cn, aws-us-gov, or ipcld (Impossible Cloud)`);
            }

            // Validate service (for S3 bucket policies, most common are IAM, S3, STS)
            const validServices = ['iam', 's3', 'sts'];
            const awsOnlyServices = ['ec2', 'lambda', 'cloudfront', 'elasticloadbalancing', 'rds', 'dynamodb'];

            if (!validServices.includes(service) && !awsOnlyServices.includes(service)) {
                warnings.push(`Service "${service}" is unusual for S3 bucket policies. Common services: iam, s3, sts`);
            } else if (awsOnlyServices.includes(service)) {
                warnings.push(`⚠ Service "${service}" is AWS-specific and not supported by Impossible Cloud. Use iam, s3, or sts instead.`);
            }
            ...
            if (service === 'iam') {
                ...
                if (!resource) {
                    ...
                    const validResourceTypes = ['user', 'role', 'group', 'instance-profile', 'root', 'federated-user', 'assumed-role'];
                    ...
                    if (resource !== 'root' && !resource.includes('/')) {
                        ...
                        warnings.push(`Resource type "${resourceType}" may not be valid. Common types: user, role, root`);
                    }
                }
        ...
        if (/^\d{12}$/.test(principal)) {
            warnings.push('Using just an account ID. Consider using full ARN format: arn:aws:iam::123456789012:root');
        } else {
            errors.push('Principal must be "*", a valid ARN (arn:aws:... or arn:ipcld:...), canonical user ID (64-char hex), service principal (service.amazonaws.com), or 12-digit account ID');
        }
    }
...
function getARNSuggestion(principal) {
    ...
    if (!principal || principal.length === 0) {
        return {
            type: 'hint',
            message: '💡 Start typing: <code>*</code> for public, <code>arn:aws:iam::</code> for AWS IAM, <code>arn:ipcld:iam::</code> for Impossible Cloud, or 64-char hex for canonical ID'
        };
    }

    // If they're typing an ARN
    if (principal.startsWith('arn:') || principal.startsWith('arn') || principal.startsWith('ar') || principal.startsWith('a')) {
        const parts = principal.split(':');
        ...
        if (parts.length === 2 && principal.endsWith(':')) {
            return {
                type: 'hint',
                message: '💡 Next: partition → <code>arn:aws:</code> for AWS, <code>arn:ipcld:</code> for Impossible Cloud'
            };
        }
        ...
        if (parts.length === 2 && !principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                ...
                return {
                    type: 'hint',
                    message: '💡 Continue: <code>ipcld</code> for Impossible Cloud or <code>aws</code> for AWS'
                };
            ...
            } else if ('aws'.startsWith(partition) || 'aws-cn'.startsWith(partition) || 'aws-us-gov'.startsWith(partition)) {
                return {
                    type: 'hint',
        ...
        if (parts.length === 3 && principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                return {
                    type: 'hint',
                    message: '💡 Next: service → <code>iam</code> (Impossible Cloud only supports IAM currently)'
                };
            } else {
                return {
                    type: 'hint',
                    message: '💡 Next: service → <code>iam</code>, <code>s3</code>, or <code>sts</code> then <code>:</code>'
                };
            }
        }
        ...
        if (parts.length === 3 && !principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                if (service === 'iam') {
                    return {
                        type: 'hint',
                        message: '💡 Good! Now add <code>:</code> then region (leave empty): <code>arn:ipcld:iam:</code>'
                    };
            ...
            } else if (['ec2', 'lambda', 'cloudfront', 'elasticloadbalancing', 'rds', 'dynamodb'].includes(service)) {
                return {
                    type: 'hint',
            ...
            } else if ('iam'.startsWith(service) || 's3'.startsWith(service) || 'sts'.startsWith(service)) {
                return {
                    type: 'hint',
        ...
        if (parts.length === 4 && principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                return {
                    type: 'hint',
                    message: '💡 Next: region (leave empty for Impossible Cloud) → <code>:</code> then canonical ID'
                };
            } else {
                return {
                    type: 'hint',
        ...
        if (parts.length === 4 && !principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                return {
                    type: 'hint',
                    message: '💡 Impossible Cloud ARNs leave region empty. Add <code>:</code> for canonical ID'
                };
            } else {
                return {
                    type: 'hint',
        ...
        if (parts.length === 5 && principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                return {
                    type: 'hint',
                    message: '💡 Next: Your Impossible Cloud canonical ID (alphanumeric string) then <code>:</code>'
                };
            } else {
                return {
                    type: 'hint',
        ...
        if (parts.length === 6 && principal.endsWith(':')) {
            ...
            if (partition === 'ipcld') {
                return {
                    type: 'hint',
                    message: '💡 Next: resource → Examples: <code>user/alice</code>, <code>user/bob</code>, <code>policy/MyBucketPolicy</code>'
                };
            } else if (service === 'iam') {
                return {
                    type: 'hint',
                    message: '💡 Next: resource → Examples: <code>user/alice</code>, <code>role/S3AccessRole</code>, <code>group/Developers</code>, or <code>root</code>'
                };
            } else {
                return {
                    type: 'hint',
                ...
                if (resource.includes('/')) {
                    ...
                    if (['user', 'policy'].includes(resourceType)) {
                        if (resourceName && resourceName.length > 0) {
                            ...
                            const examples = resourceType === 'user'
                    ...
                    return {
                        type: 'hint',
                        message: '💡 Continue typing: <code>user/alice</code>, <code>user/bob</code>, or <code>policy/MyBucketPolicy</code>'
                    };
                }
            } else if (service === 'iam') {
                if (resource === 'root') {
                    ...
                    } else if ('user'.startsWith(resourceType) || 'role'.startsWith(resourceType) || 'group'.startsWith(resourceType)) {
                        return {
                            type: 'hint',
                            message: '💡 Common types: <code>user/</code>, <code>role/</code>, <code>group/</code>'
                        };
                    }
                } else if ('user'.startsWith(resource) || 'role'.startsWith(resource) || 'group'.startsWith(resource) || 'root'.startsWith(resource)) {
                    return {
                        type: 'hint',
                        message: '💡 Continue typing: <code>user/alice</code>, <code>role/S3AccessRole</code>, <code>group/Developers</code>, or <code>root</code>'
                    };
                }
            }
        }
    ...
    if (principal.includes('.') || principal.includes('amazonaws')) {
        ...
        } else if ('s3.amazonaws.com'.startsWith(principal) ||
    ...
    if (/^\d+$/.test(principal)) {
        if (principal.length < 12) {
            ...
            return {
                type: 'warning',
                message: '⚠ Valid account ID. Consider using full ARN: <code>arn:aws:iam::' + principal + ':root</code>'
            };
        } else {
            return {
                type: 'error',
...
function generatePolicy() {
    ...
    if (!bucketValidation.isValid) {
        ...
        showNotification('Invalid bucket name. Please fix the errors shown below the bucket name field.', 'error');
    ...
    const resources = [];
    const needsBucketResource = actions.some(a => a === 's3:ListBucket' || a.startsWith('s3:GetBucket') || a.startsWith('s3:PutBucket'));
...
function validateStatement(stmt, idx, errors, warnings) {
    ...
    if (stmt.Principal) {
        warnings.push(`${prefix}: Principal field is not supported by Impossible Cloud and will be ignored`);
    }
    if (stmt.NotPrincipal) {
        warnings.push(`${prefix}: NotPrincipal field is not supported by Impossible Cloud and will be ignored`);
    }
    ...
    const validStmtFields = ['Sid', 'Effect', 'Principal', 'NotPrincipal', 'Action', 'NotAction', 'Resource', 'NotResource', 'Condition'];
...
function validatePrincipal(principal, prefix, errors, warnings) {
    if (principal === '*') {
        warnings.push(`${prefix}: Using wildcard (*) grants public access - ensure this is intentional`);
        return;
    }

    if (typeof principal === 'string') {
        errors.push(`${prefix}: String principals other than "*" are not valid. Use {"AWS": "..."} format`);
        return;
    }
    ...
    Object.keys(principal).forEach(key => {
        if (!validPrincipalKeys.includes(key)) {
            errors.push(`${prefix}: Unknown principal type "${key}". Valid types: ${validPrincipalKeys.join(', ')}`);
        }
    });
    ...
    if (principal.Service) {
        ...
        services.forEach((svc, idx) => {
            if (typeof svc !== 'string') {
                ...
                warnings.push(`${prefix}.Service[${idx}]: Service principal should typically end with .amazonaws.com`);
            }
        });
    }
}
...
function validateActions(actions, prefix, errors, warnings) {
    ...
    const validS3Actions = [
        's3:*',
        's3:GetObject', 's3:PutObject', 's3:DeleteObject', 's3:GetObjectVersion', 's3:DeleteObjectVersion',
        's3:GetObjectAcl', 's3:PutObjectAcl', 's3:GetObjectVersionAcl', 's3:PutObjectVersionAcl',
        ...
        's3:GetObjectTorrent', 's3:GetObjectVersionTorrent',
        's3:GetObjectAttributes', 's3:GetObjectVersionAttributes',
        's3:ListBucket', 's3:ListBucketVersions', 's3:ListBucketMultipartUploads',
        's3:GetBucketLocation', 's3:GetBucketVersioning', 's3:PutBucketVersioning', 's3:GetBucketAcl', 's3:PutBucketAcl',
    ...
    actionList.forEach((action, idx) => {
        if (typeof action !== 'string') {
            ...
            } else if (service === 's3' && actionName !== '*' && !validS3Actions.includes(action) && !actionName.includes('*')) {
...
function validateCondition(condition, prefix, errors, warnings) {
    ...
    const validConditionOperators = [
        ...
        'StringEqualsIfExists', 'StringNotEqualsIfExists', 'StringLikeIfExists', 'StringNotLikeIfExists',
        'StringEqualsIgnoreCaseIfExists', 'StringNotEqualsIgnoreCaseIfExists',
        // Numeric operators
        'NumericEquals', 'NumericNotEquals', 'NumericLessThan', 'NumericLessThanEquals',
        'NumericGreaterThan', 'NumericGreaterThanEquals',
        // Numeric operators with IfExists
        'NumericEqualsIfExists', 'NumericNotEqualsIfExists', 'NumericLessThanIfExists', 'NumericLessThanEqualsIfExists',
        ...
        'DateEqualsIfExists', 'DateNotEqualsIfExists', 'DateLessThanIfExists', 'DateLessThanEqualsIfExists',
        ...
        'ArnEqualsIfExists', 'ArnLikeIfExists', 'ArnNotEqualsIfExists', 'ArnNotLikeIfExists',
        // Null check
        'Null'
    ];
    ...
    Object.keys(condition).forEach(operator => {
        ...
        if (typeof conditionBlock !== 'object' || conditionBlock === null || Array.isArray(conditionBlock)) {
...
window.onload = function() {
    ...
    // Add real-time validation for principal ARN
    const principalInput = document.getElementById('principal');
